...
{
  FLASH_Status FLASHStatus = FLASH_COMPLETE;    //操作完成

  uint32_t i;

  if(StartAddress<APP2BOOT_ADDR){         //写入地址是否正确
  ...
  {
    FLASHStatus = FLASH_ProgramWord(StartAddress, *((uint32_t*)pData));//向起始地址32位写入函数
    if (FLASHStatus == FLASH_COMPLETE){    //是否写入完成
      StartAddress += 4;
      pData += 4;
    }else{ 
      return FLASHStatus;
...
/**
  ...
  * @retval 扇区擦出状态  
  */
FLASH_Status BOOT_ErasePage(uint32_t StartPageAddr,uint32_t EndPageAddr)     //指定扇区FLASH擦除函数
{
  uint32_t i;
...
void JumpToApplication(uint32_t Addr)
{
  ...
  if (((*(__IO uint32_t*)Addr) & 0x2FFE0000 ) == 0x20000000)         //按位取与
  { 
